...
export async function ensureGalleryPosts(urls, category = "flashback") {
  ...
  const hiddenRes = await query("SELECT image_url FROM gallery_hidden");
  const hidden = new Set(hiddenRes.rows.map((r) => r.image_url));
  const keep = urls.filter((u) => !hidden.has(u));
  if (!keep.length) return;
  const catParam = keep.length + 1;
  const values = keep.map((_, i) => `($${i + 1}, $${catParam})`).join(",");
  await query(
