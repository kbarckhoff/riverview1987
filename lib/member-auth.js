...
export async function getCurrentMember() {
  ...
  if (!id) return null;
  const { rows } = await query("SELECT id, email, name FROM members WHERE id=$1", [id]);
  const m = rows[0];
  if (!m) return null;
  // Admin is decided ONLY by the email allowlist in site-config (live).
  const admins = (site.adminEmails || []).map((e) => e.toLowerCase());
  m.is_admin = admins.includes((m.email || "").toLowerCase());
  return m;
}
